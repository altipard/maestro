    ...
    for piece in pieces:
        ...
        if current and current_len + piece_len > chunk_size:
            ...
            if chunk_overlap > 0:
                ...
                current_len = sum(len(p) for p in current) + len(chosen_sep) * max(len(current) - 1, 0)
            else:
                current = []
                current_len = 0
    ...
    if current:
        ...
        if merged:
            ...
            if len(merged) > chunk_size and remaining_seps:
                chunks.extend(
                    _recursive_split(merged, remaining_seps, chunk_size, chunk_overlap)
                )
            else:
                chunks.append(merged)
...
class TextSegmenter:
    ...
    def __init__(
        ...
        **_: object,
        ...
        try:
            from semantic_text_splitter import MarkdownSplitter as _MS  # noqa: N813
            from semantic_text_splitter import TextSplitter as _TS  # noqa: N813
            self._semantic_md = _MS
            self._semantic_txt = _TS
